...
To go forward, I really need better javadoc integration. [[https://github.com/senny/emacs-eclim][Emacs Eclim]] provides that: Eclipse editing features as server, so I can use them from Emacs without having to use the Eclipse User Interface.
